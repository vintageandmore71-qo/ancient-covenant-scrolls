...
| **X-AI-CHAT-STUDIO** | **Load AI Chat Studio** &mdash; conversational generation surface where outputs appear directly inside the chat thread. Lives in **`/loadstudio/`**, not Load main. Required behaviour: user types natural-language request &rarr; system generates image inline &rarr; system provides Save / Export / Feedback / Attach to Project / Attach to Scene actions &rarr; user can refine in same thread &rarr; user can ask to animate &rarr; system shows progress card &rarr; system returns playable video inline &rarr; user saves / extends / refines / replaces audio / adds more SFX / attaches to scene. **Two animation levels:** Level 1 light (camera push-in, parallax, soft motion); Level 2 performance (head turns, facial expressions, body / hair / clothing motion, identity preservation). **Five required UI cards:** image result, animation pending, audio generation, video result, audio result. **Twenty required chat features** including text-to-image, image-to-image, multi-style, style selector, reference image upload, regenerate / variation, save to project, attach to scene, animate this, image-to-video or animation-ready prompt, motion prompt input, prompt-only fallback. Splits a single user request into visual / motion / camera / character / environment / sound / music / SFX prompts; routes through X-AI-CORE; runs animation + audio jobs in parallel. **Do not build:** beauty filters, glamour effects, viral social effects, AI photoshoots as the main product, face retouching, makeover tools, entertainment-first trend features. Build only a Load-native creation surface for content / movies / books / scripts / PWA cinema packages. Suggested modules: `/loadstudio/ai-chat/index.html`, `chat-studio.js`, `chat-history.js`, `output-selector.js`, `style-selector.js`, `reference-upload.js`, `image-generator.js`, `image-refiner.js`, `image-animator.js`, `project-attach.js`, `chat-export.js`. | `inbox/5.7 load_ai_chat_animation.zip` + `inbox/Load Main AI Addendum.docx` + Combined ZIP Part 3 (2026-05-06/07). **VERY IMPORTANT.** | **Not started.** | Depends on X-AI-PROVIDERS (registry + audit) + X-AI-CORE expansion (project memory + output proof). Writes into `chat-history.json`, `prompt-log.json`, `generation-report.json`, `rights.json`, `asset-declarations.json`, `scene-plan.json`, `continuity-report.json`. Output proof: never claim image / animation / video / audio success unless real playable file/blob/URL exists. |
| **X-STUDIO-AI** | **Load Creation Engine for AI Production** &mdash; LoadStudio's creative cockpit. Uses Load Main's X-AI-CORE to produce structured scenes, prompts, characters, voices, styles, and exportable packages. Required screens: Load Creation Engine, AI Scene Builder, Book-to-Cinema, Script-to-Shot, Character Bible generator, Style Bible generator, Voice Bible generator, Wardrobe Continuity tracker, Prompt Studio, Image Prompt generator, Video Prompt generator, Music/SFX prompt generator, Scene Consistency checker, Character Consistency checker, Continuity Check, Generation Queue, Output Review. Produces: `scenes.json`, `characters.json`, `rights.json`, `credits.json`, `style-bible.json`, `voice-bible.json`, `wardrobe-bible.json`, `prompt-log.json`, `generation-report.json`, `continuity-report.json`, `model-routing.json`, `asset-declarations.json`, `chat-history.json`. Exports `.loadstudio.zip` and `.cinepwa.zip` packages compatible with the Load Main validator and LoadPlay publish-prep. Connects to **X-VIDEO-AI** so AI video generation is not a separate disconnected system. Allows AI-built projects to be reopened and edited inside LoadStudio. **Do not lock LoadStudio to one provider.** Always allow prompt-only fallback if generation fails. Require output proof before claiming generation succeeded. | Combined ZIP Part 2 + addendum (2026-05-06/07). **VERY IMPORTANT.** | **Not started.** | Depends on X-AI-CORE expansion (the model router LoadStudio calls into) + X-AI-PROVIDERS (registry) + X-AI-CHAT-STUDIO (the chat surface that drives the creation engine). Output proof rules apply equally. |
...
- **Load AI Chat Studio** &mdash; see **X-AI-CHAT-STUDIO**. Captured 2026-05-06/07 as VERY IMPORTANT. Conversational generation surface in `/loadstudio/`, not Load main. Two animation levels (light camera/parallax + performance head/face/body), five required UI cards, twenty required chat features, splits user request into visual / motion / sound / music / SFX prompts. **Do not build** beauty filters / glamour / viral social effects / face retouching. Output proof rules apply.
- **Load Creation Engine for AI Production** &mdash; see **X-STUDIO-AI**. Captured 2026-05-06/07 as VERY IMPORTANT. LoadStudio creative cockpit covering AI Scene Builder, Book-to-Cinema, Script-to-Shot, Character Bible / Style Bible / Voice Bible generators, Wardrobe Continuity, Prompt Studio, Generation Queue, Output Review. Produces 13 structured project files. Exports `.loadstudio.zip` / `.cinepwa.zip`. Reuses Load Main's X-AI-CORE; never locks to one provider; prompt-only fallback always allowed.
