...
// Pipeline order: (1) local/browser OSS → (2) no-key free cloud → (3) key free-tier → (4) free-credits optional → (5) paid-optional (user-enabled only)
// BLOCKED providers are never placed in chains. Paid-optional providers at chain tail; skipped unless user has enabled paid connectors.
var _FALLBACK_CHAINS = {
  'image':        ['pollinations-image', 'aihorde', 'flux', 'flux-1-schnell', 'sdxl', 'sd-15', 'kolors', 'auraflow', 'stable-cascade', 'playground-v25', 'pixazo', 'huggingface', 'comfyui', 'a1111'],
  'audio':        ['kokoro', 'chatterbox', 'chatterbox-turbo', 'f5-tts', 'bark', 'openvoice', 'xtts-v2', 'piper', 'coqui', 'dia', 'elevenlabs', 'browser-tts'],
  'video':        ['wan', 'hunyuanvideo', 'ltx-video', 'animatediff', 'cogvideox', 'framepack', 'cosmos-predict', 'open-sora', 'mochi', 'dynamicrafter', 'i2vgen-xl', 'svd', 'zsky', 'kling', 'hailuo', 'luma-dream', 'pika', 'pixverse'],
  ...
  'sfx':          ['audiogen', 'openverse-sfx', 'freesound'],
  'stock':        ['wikimedia', 'openverse', 'nasa-library', 'internet-archive', 'coverr', 'pexels', 'pixabay-stock'],
  'llm':          ['pollinations-text', 'ollama', 'gemini', 'groq', 'openrouter', 'cerebras', 'sambanova', 'together-ai', 'deepinfra']
};

// ─── FAILURE TRACKING / COOLDOWN ─────────────────────────────────────────────
// Reason codes: quota_exceeded | rate_limited | auth_missing | provider_error | no_output | timeout
var _COOLDOWN_MS = 5 * 60 * 1000; // 5 minutes
var _cooldowns   = {};  // { providerId: { until: ms, reason: string } }
var _lastFailure = {};  // { providerId: reason string }

function _setCooldown(providerId, reason) {
  _cooldowns[providerId]   = { until: Date.now() + _COOLDOWN_MS, reason: reason };
  _lastFailure[providerId] = reason;
}

function _isCoolingDown(providerId) {
  var c = _cooldowns[providerId];
  if (!c) return false;
  if (Date.now() >= c.until) { delete _cooldowns[providerId]; return false; }
  return true;
}

function _classifyFailure(err) {
  var msg = err ? (err.message || '').toLowerCase() : '';
  if (msg.indexOf(' 429') !== -1 || msg.indexOf('rate') !== -1 || msg.indexOf('too many') !== -1) return 'rate_limited';
  if (msg.indexOf(' 402') !== -1 || msg.indexOf('quota') !== -1 || msg.indexOf('exceeded') !== -1 || msg.indexOf('billing') !== -1) return 'quota_exceeded';
  if (msg.indexOf(' 401') !== -1 || msg.indexOf(' 403') !== -1 || msg.indexOf('auth') !== -1 || msg.indexOf('no api key') !== -1 || msg.indexOf('no connector') !== -1) return 'auth_missing';
  if (msg.indexOf('timeout') !== -1) return 'timeout';
  if (msg.indexOf('no output') !== -1 || msg.indexOf('empty') !== -1) return 'no_output';
  return 'provider_error';
}

// Tier labels for UI classification (derived at runtime from provider flags)
function _getTierLabel(p) {
  if (!p) return 'UNKNOWN';
  if (p.status === 'blocked') return 'BLOCKED';
  if (p.isFree === false && !p.isOpenSource) return 'PAID_OPTIONAL';
  if (p.accessType === 'browser-api' || p.accessType === 'wasm') return 'FREE_OPEN';
  if (p.isOpenSource && p.isLocal) return 'FREE_OPEN';
  if (!p.requiresApiKey && !p.requiresLocalEndpoint && p.isFree) return 'FREE_OPEN';
  if (p.requiresApiKey && p.isFree && (p.costLabel || '').toLowerCase().indexOf('credit') !== -1) return 'FREE_CREDITS_OPTIONAL';
  if (p.requiresApiKey && p.isFree) return 'FREE_TIER';
  if (p.requiresApiKey) return 'AUTH_REQUIRED';
  if (p.requiresLocalEndpoint) return 'FREE_OPEN';
  return 'FREE_TIER';
}
...
var _PROVIDERS_EXTRA = [
  ...
  // ─── NEW PROVIDERS — added 2026-05-18 ────────────────────────────────────────
  // Video generation
  { providerId:'framepack',           name:'FramePack',             aliases:['Frame Pack'],          category:'video-gen',        capabilities:['video-generation','video-to-video','frame-prediction','scene-extension'], accessType:'local-endpoint', requiresApiKey:false, requiresLocalEndpoint:true, defaultEndpoint:'http://localhost:7860', isFree:true, isOpenSource:true, isLocal:true, isBrowserNative:false, isHosted:false, isOpenAICompatible:false, requiresAccount:false, requiresBackendProxy:false, privacyLabel:'offline', licenseRisk:'low', commercialUseStatus:'check-required', license:'Apache 2.0', costLabel:'Free', fallbackEligible:true, status:'untested', blockedReason:null, websiteUrl:'https://github.com/lllyasviel/FramePack', docsUrl:'https://github.com/lllyasviel/FramePack', whereUsedInLoadStudio:['AI Image Director'], whereUsedInLoadEco:false, futureLoadAIUse:'video-generation', testAction:'test-local-endpoint', settingsAction:'set-endpoint', useAsPrimary:false, useAsFallback:true, notes:'Scene extension / frame prediction. Runs via ComfyUI or Gradio.', pipelineMembership:['ai-video','local-selfhosted-ai'], inputTypes:['image','video'], outputTypes:['video'] },
  { providerId:'cosmos-predict',      name:'COSMOS Predict',        aliases:['NVIDIA COSMOS'],       category:'video-gen',        capabilities:['video-generation','world-model','physics-aware'],               accessType:'local-endpoint',  requiresApiKey:false, requiresLocalEndpoint:true,  defaultEndpoint:'http://localhost:7860', isFree:true,  isOpenSource:true,  isLocal:true,  isBrowserNative:false, isHosted:false, isOpenAICompatible:false, requiresAccount:false, requiresBackendProxy:false, privacyLabel:'offline',           licenseRisk:'medium', commercialUseStatus:'check-required',  license:'NVIDIA Open Model License', costLabel:'Free', fallbackEligible:false, status:'untested', blockedReason:null, websiteUrl:'https://github.com/NVIDIA/Cosmos', docsUrl:'https://github.com/NVIDIA/Cosmos', whereUsedInLoadStudio:['AI Image Director'], whereUsedInLoadEco:false, futureLoadAIUse:'video-generation', testAction:'test-local-endpoint', settingsAction:'set-endpoint', useAsPrimary:false, useAsFallback:false, notes:'NVIDIA world foundation model for video prediction. Requires GPU.', pipelineMembership:['ai-video','local-selfhosted-ai'], inputTypes:['text','image'], outputTypes:['video'] },
  // Image generation
  { providerId:'kolors',              name:'Kolors',                aliases:['Kwai Kolors'],         category:'image-gen',        capabilities:['image-generation','chinese-text','high-quality'],               accessType:'local-endpoint',  requiresApiKey:false, requiresLocalEndpoint:true,  defaultEndpoint:'http://localhost:8188', isFree:true,  isOpenSource:true,  isLocal:true,  isBrowserNative:false, isHosted:false, isOpenAICompatible:false, requiresAccount:false, requiresBackendProxy:false, privacyLabel:'offline',           licenseRisk:'low',    commercialUseStatus:'check-required',  license:'Apache 2.0',   costLabel:'Free',         fallbackEligible:true,  status:'untested', blockedReason:null, websiteUrl:'https://huggingface.co/Kwai-Kolors/Kolors', docsUrl:'https://huggingface.co/Kwai-Kolors/Kolors', whereUsedInLoadStudio:['AI Image Director'], whereUsedInLoadEco:false, futureLoadAIUse:'image-generation', testAction:'test-local-endpoint', settingsAction:'set-endpoint', useAsPrimary:false, useAsFallback:true, notes:'Kwai open-weight model. Strong at photorealistic and artistic styles. Runs via ComfyUI.', pipelineMembership:['ai-image-glamour','local-selfhosted-ai'], inputTypes:['text'], outputTypes:['image'] },
  { providerId:'auraflow',            name:'AuraFlow',              aliases:['Aura Flow'],           category:'image-gen',        capabilities:['image-generation','flow-matching','open-weights'],              accessType:'local-endpoint',  requiresApiKey:false, requiresLocalEndpoint:true,  defaultEndpoint:'http://localhost:8188', isFree:true,  isOpenSource:true,  isLocal:true,  isBrowserNative:false, isHosted:false, isOpenAICompatible:false, requiresAccount:false, requiresBackendProxy:false, privacyLabel:'offline',           licenseRisk:'none',   commercialUseStatus:'permitted',       license:'Apache 2.0',   costLabel:'Free',         fallbackEligible:true,  status:'untested', blockedReason:null, websiteUrl:'https://huggingface.co/fal/AuraFlow', docsUrl:'https://huggingface.co/fal/AuraFlow', whereUsedInLoadStudio:['AI Image Director'], whereUsedInLoadEco:false, futureLoadAIUse:'image-generation', testAction:'test-local-endpoint', settingsAction:'set-endpoint', useAsPrimary:false, useAsFallback:true, notes:'Flow-matching T2I. Apache 2.0 — fully commercial-safe.', pipelineMembership:['ai-image-glamour','local-selfhosted-ai'], inputTypes:['text'], outputTypes:['image'] },
  { providerId:'stable-cascade',      name:'Stable Cascade',        aliases:['SD Cascade'],          category:'image-gen',        capabilities:['image-generation','two-stage-cascade','high-res'],             accessType:'local-endpoint',  requiresApiKey:false, requiresLocalEndpoint:true,  defaultEndpoint:'http://localhost:8188', isFree:true,  isOpenSource:true,  isLocal:true,  isBrowserNative:false, isHosted:false, isOpenAICompatible:false, requiresAccount:false, requiresBackendProxy:false, privacyLabel:'offline',           licenseRisk:'low',    commercialUseStatus:'check-required',  license:'Stability AI Community License', costLabel:'Free', fallbackEligible:true, status:'untested', blockedReason:null, websiteUrl:'https://huggingface.co/stabilityai/stable-cascade', docsUrl:'https://huggingface.co/stabilityai/stable-cascade', whereUsedInLoadStudio:['AI Image Director'], whereUsedInLoadEco:false, futureLoadAIUse:'image-generation', testAction:'test-local-endpoint', settingsAction:'set-endpoint', useAsPrimary:false, useAsFallback:true, notes:'Two-stage diffusion — efficient high-res. Runs via ComfyUI.', pipelineMembership:['ai-image-glamour','local-selfhosted-ai'], inputTypes:['text','image'], outputTypes:['image'] },
  ...
  { providerId:'leonardo-ai',         name:'Leonardo AI',           aliases:['Leonardo'],            category:'image-gen',     capabilities:['image-generation','game-assets','characters','environments'],   accessType:'free-api-key',    requiresApiKey:true,  requiresLocalEndpoint:false, defaultEndpoint:null,                     isFree:true,  isOpenSource:false, isLocal:false, isBrowserNative:false, isHosted:true,  isOpenAICompatible:false, requiresAccount:true,  requiresBackendProxy:false, privacyLabel:'third-party-cloud',  licenseRisk:'low',    commercialUseStatus:'check-required',  license:'Proprietary',  costLabel:'Free (daily gen)', fallbackEligible:false, status:'blocked', blockedReason:'user rule: do not re-add to active routing', websiteUrl:'https://leonardo.ai', docsUrl:'https://docs.leonardo.ai', whereUsedInLoadStudio:[], whereUsedInLoadEco:false, futureLoadAIUse:'none', testAction:'none', settingsAction:'none', useAsPrimary:false, useAsFallback:false, notes:'BLOCKED — kept for audit history only.', pipelineMembership:[], inputTypes:['text'], outputTypes:['image'] },
  ...
  { providerId:'replicate',           name:'Replicate',             aliases:[],                      category:'aggregator',       capabilities:['image-generation','video-generation','tts','multi-model'],      accessType:'paid-api-key',    requiresApiKey:true,  requiresLocalEndpoint:false, defaultEndpoint:'https://api.replicate.com/v1', isFree:false, isOpenSource:false, isLocal:false, isBrowserNative:false, isHosted:true, isOpenAICompatible:false, requiresAccount:true, requiresBackendProxy:false, privacyLabel:'third-party-cloud', licenseRisk:'low', commercialUseStatus:'check-required', license:'Proprietary', costLabel:'Pay-per-inference', fallbackEligible:false, status:'blocked', blockedReason:'pay-per-inference — no sustained free tier; not eligible for LoadStudio default routing', websiteUrl:'https://replicate.com', docsUrl:'https://replicate.com/docs', whereUsedInLoadStudio:[], whereUsedInLoadEco:false, futureLoadAIUse:'none', testAction:'none', settingsAction:'none', useAsPrimary:false, useAsFallback:false, notes:'BLOCKED from default routing — pay-per-inference. Kept for audit history.', pipelineMembership:[], inputTypes:['text','image'], outputTypes:['image','video','audio'] },
  ...
  { providerId:'tensor-art',          name:'Tensor.Art',            aliases:['TensorArt'],           category:'image-gen',        capabilities:['text-to-image','img2img','LoRA','model-hosting'],                  accessType:'freemium',        requiresApiKey:true,  requiresLocalEndpoint:false, defaultEndpoint:null,                     isFree:true,  isOpenSource:false, isLocal:false, isBrowserNative:false, isHosted:true,  isOpenAICompatible:false, requiresAccount:true,  requiresBackendProxy:false, privacyLabel:'third-party-cloud',  licenseRisk:'low',    commercialUseStatus:'check-required',  license:'Proprietary',  costLabel:'Free tier',    fallbackEligible:false, status:'blocked', blockedReason:'user rule: do not re-add to active routing', websiteUrl:'https://tensor.art', docsUrl:'https://tensor.art', whereUsedInLoadStudio:[], whereUsedInLoadEco:false, futureLoadAIUse:'none', testAction:'none', settingsAction:'none', useAsPrimary:false, useAsFallback:false, notes:'BLOCKED — kept for audit history only.', pipelineMembership:[], inputTypes:['text','image'], outputTypes:['image'] },
...
var LoadProviderRegistry = {
  ...
  routeToFallback: function (request) {
    ...
    // Skip blocked providers and providers currently in cooldown
    var candidates = chain.slice(startIdx).filter(function(id) {
      if (id === failed) return false;
      if (_isCoolingDown(id)) return false;
      var meta = _PROVIDER_META[id] || self.getProvider(id);
      if (meta && meta.status === 'blocked') return false;
      return true;
    });
    ...
    function tryNext(idx) {
      ...
      return fn(req).catch(function(err) {
        if (_isQuotaError(err)) {
          _setCooldown(id, _classifyFailure(err));
  ...
  callLLM: function (request) {
    ...
    var _cloudRoutes = {
      ...
      // mistral: BLOCKED — caused AI/provider loading issues in prior testing
...
// Tier classification and cooldown inspection (read-only, safe to call from UI)
LoadProviderRegistry.getTier = function (providerIdOrObj) {
  var p = (typeof providerIdOrObj === 'string') ? this.getProvider(providerIdOrObj) : providerIdOrObj;
  return _getTierLabel(p);
};
LoadProviderRegistry.isCoolingDown = function (providerId) { return _isCoolingDown(providerId); };
LoadProviderRegistry.getLastFailureReason = function (providerId) { return _lastFailure[providerId] || null; };
LoadProviderRegistry.getCooldowns = function () {
  var now = Date.now();
  var out = {};
  Object.keys(_cooldowns).forEach(function(id) {
    var c = _cooldowns[id];
    if (now < c.until) out[id] = { reason: c.reason, remainingMs: c.until - now };
  });
  return out;
};
    ...
    LoadProviderRegistry[name] = function(request) {
      ...
      return orig.call(self, request).catch(function(err) {
        if (_isQuotaError(err) && _FALLBACK_CHAINS[chainType] && _FALLBACK_CHAINS[chainType].length) {
          var failedId = (request && request.providerId) || null;
          if (failedId) _setCooldown(failedId, _classifyFailure(err));
          return self.routeToFallback({
            type: chainType,
            originalProviderId: failedId,
